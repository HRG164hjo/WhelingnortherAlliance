...
namespace WNA.WNAHediffCompProp
{
    ...
    {
        public int healTickerMin = 200;

        public int healTickerMax = 300;
    ...
    public class CompHealPermaWounds : HediffComp
    {
        ...
        {
            ticksToHeal = Rand.Range(Props.healTickerMin, Props.healTickerMax);
        }
        ...
        {
            if (pawn.health.hediffSet.hediffs.Where(hd => hd.IsPermanent() || hd.def.chronic).TryRandomElement(out var result))
            {
